      ...
      try {
        ...
        if (isEdit) {
          ...
          if (manifest) {
            ...
            const fileContents = (global.sandboxState && global.sandboxState.fileCache && global.sandboxState.fileCache.files) ? global.sandboxState.fileCache.files : {};
              ...
              if (filesResponse.ok) {
                ...
                if (filesData.success && filesData.files) {
                  ...
                  for (const [path, content] of Object.entries(filesData.files)) {
                    ...
                    if (!global.sandboxState || !global.sandboxState.fileCache) {
                      if (!global.sandboxState) {
                        (global as any).sandboxState = { fileCache: { files: {}, lastSync: Date.now(), sandboxId: context?.sandboxId || 'unknown' } };
                      } else {
                  ...
                  // Update variables
                  backendFiles = (global.sandboxState && global.sandboxState.fileCache) ? global.sandboxState.fileCache.files : {};
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const completionResult = await streamText({
                  model: completionClient(((global as any).modelMapping as any)?.[model] || model),
                  messages: [
                    { 
                      role: 'system', 
                  ...
                  temperature: (global as any).isGPT5 ? undefined : appConfig.ai.defaultTemperature,
